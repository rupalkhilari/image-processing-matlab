...
% Topic : Butterworth Low Pass filtering 
% Name : Rupal K 
% Class : Msc IT - Part 1 
...
M=s(1);
N=s(2);
D0=input('Enter the threshold frequency : ');
...
sum=term1+term2;
D(u,v)=sqrt(sum);
        temp1=power(double(D(u,v)/D0),2*n);
            H(u,v)=1/(1+temp1);
    end
end
...
figure;
imshow(realG,[]);
title(['Butterworth Low Pass Filtering -radius :',num2str(D0),' ,Order : ',num2str(n)]);
